...
const CheapestPlansComponent = ({ searchParams, params }: any) => {
  ...
  const { data, isLoading, isError, error }: any = useQuery("get-high-speed-plans", async () => {
    ...
    try {
      ...
      const APIURL = `${clientSide.REACT_APP_API_DOMAIN_URL}${clientSide.REACT_API_HIGHSPEEDPLAN_PATH}`;
      const APIURL_META = `${clientSide.REACT_APP_API_DOMAIN_URL}${clientSide.REACT_API_PAGE_KEY_BASE}highspeedplan`;
