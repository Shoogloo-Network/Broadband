...
import { 
  API_PAGE_KEY_BASE,
  API_PLAN_LATEST_OFFER,
  API_PLAN_TOP_PROVIDERS,
  api 
} from '@/config';
...
const getCountryData = async (pageSiteId: any) => {
  try {
    const headersList = headers();
    // /api/blog
    const reqHeadersHost: any = headersList.get('host');
    const blogViewPostAPIURLOrigin =
      (reqHeadersHost == 'localhost:3000' || reqHeadersHost == 'localhost:3001') ? 'http://' : 'https://';
...
const CountryPage = async ({ searchParams, params }: any) => {
  
  const pageSiteId = searchParams.pageSiteId;
  const data = await getCountryData(pageSiteId);
